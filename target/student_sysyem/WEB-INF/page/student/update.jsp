                    ...
                    <select name="subjectId" lay-filter="subjectId" lay-verify="required">
                        ...
                        <c:forEach items="${subjects}" var="subject">
                            <option value="${subject.id}"
                                    <c:if test="${student.subjectId == subject.id}">selected</c:if>
...
<script>
    layui.use(['form', 'jquery', 'laydate'], function () {
        ...
        // 初始化的时候加载一次（初始化的时候不用加参数）
        initClazzList();

        function initClazzList(data) {
            $('#clazzId').empty(); // 请求一次，清空一次，不然选择框会越堆越多
            var that = data;
            var subjectId;
            if (!data) {
        ...
        // 专业和班级是有一定关系的，必须先选择专业之后才能选择班级，因此通过异步的方式实现，先捕获到form的选择事件
